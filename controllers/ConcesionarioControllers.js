...
const obtener_vehiculos = async (req, res) => {
  ...
  const obtener_vehiculos_id = async (req, res) => {
    ...
    try {
      const query = `SELECT * FROM Concesionario WHERE id = '${vehiculoId}'`;
      const result = await db.execute(query);
  ...
  const crear_vehiculo = async (req, res) => {
    ...
    try {
      const query = `INSERT INTO Concesionario (id, marca, cilindraje, combustible, ano) VALUES(default, '${marca}', '${cilindraje}', '${combustible}', '${ano}')`;
  ...
  const actualizar_vehiculo = async (req, res) => {
    ...
    const { id, marca, cilindraje, combustible, ano } = req.body;
    try {
      const query = `UPDATE Concesionario SET id = '${id}', marca = '${marca}', cilindraje = '${cilindraje}' , combustible = '${combustible}', ano = ${ano} WHERE id = '${vehiculoId}'`;
      await db.execute(query);
  ...
  const eliminar_vehiculo = async (req, res) => {
    const vehiculoId = req.params.id;
    try {
      const query = `DELETE FROM Concesionario WHERE id = '${vehiculoId}';`;
      await db.execute(query);
